...
- **Ticket Support System:** Staff/admins can use `/ticketsupportenable` (requires log channel configured) to enable the system, then `/ticketsupportsetup` to configure it. Setup includes: selecting a channel for the ticket panel, adding custom ticket types, assigning roles that can view each ticket type. When users click a button on the panel, they enter a description modal to provide details about their issue. A private ticket channel is created only visible to the user and assigned staff roles, with the ticket description displayed. Tickets are logged and tracked in the database. After each setup step, the menu automatically returns to the main setup menu for convenience.
