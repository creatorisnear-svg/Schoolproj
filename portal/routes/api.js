...
// ── Models (imported from main project) ────────────────────────────────────
...
export function createApiRouter() {
      ...
      try {
        ...
        roleDetails = rolesRes.data
          ...
          .map(r => ({
            id: r.id,
            name: r.name,
            color: r.color ? `#${r.color.toString(16).padStart(6, '0')}` : '#555555',
          }));
      } catch { /* skip */ }
  ...
  router.get('/cad', portalAuth, async (req, res) => {
    try {
      ...
      console.error('[PORTAL API /cad]', err.message);
      res.status(500).json({ error: 'Internal error' });
    }
  });
  ...
  router.post('/cad/create', portalAuth, async (req, res) => {
    try {
      ...
      console.error('[PORTAL API /cad/create]', err.message);
  ...
  router.post('/cad/:charId/vehicle', portalAuth, async (req, res) => {
    try {
      ...
      console.error('[PORTAL API /cad/vehicle]', err.message);
  ...
  router.get('/economy', portalAuth, async (req, res) => {
    try {
      ...
      console.error('[PORTAL API /economy]', err.message);
      res.status(500).json({ error: 'Internal error' });
    }
  });
  ...
  router.post('/economy/buy', portalAuth, async (req, res) => {
    try {
      ...
      console.error('[PORTAL API /economy/buy]', err.message);
  ...
  router.get('/economy/leaderboard', portalAuth, async (req, res) => {
    try {
      ...
      console.error('[PORTAL API /economy/leaderboard]', err.message);
      res.status(500).json({ error: 'Internal error' });
    }
  });
    ...
    try {
      ...
      for (const memberId of (roleType.approverMemberIds || [])) {
        try {
          ...
          const name = m?.nick || 'Unknown';
          approvers.push({ id: memberId, name });
        } catch { /* skip */ }
      }
      ...
      if (roleType.approverRoleIds?.length > 0) {
        try {
          ...
          for (const m of membersRes.data) {
            ...
            if (approvers.find(a => a.id === m.user.id)) continue;
  ...
  router.post('/rolerequest/submit', portalAuth, async (req, res) => {
    try {
      ...
      const newRequest = new RoleRequest({
        ...
        approverId,
        approverUsername: approverMember.nick || approverUser.global_name || approverUser.username || approverId,
        timestamp: new Date(),
      });
      await newRequest.save();

      // DM the approver via Discord API
      try {
        ...
        await axios.post(
          `https://discord.com/api/v10/channels/${dmChannelId}/messages`,
          {
            embeds: [{
              color: 0x2d2d2d,
              title: 'Role Request Approval',
              description: `**${req.portalUser.username}** has requested the role **${roleType.roleName}** via the Member Portal.`,
              fields: [
                { name: 'Requester', value: req.portalUser.username, inline: true },
                { name: 'Role', value: roleType.roleName, inline: true },
                { name: 'Request ID', value: requestId, inline: false },
              ],
              footer: { text: 'RPM — Use /rolerequest in Discord to approve or deny' },
            }],
          },
          { headers: { Authorization: `Bot ${process.env.DISCORD_TOKEN}`, 'Content-Type': 'application/json' } }
        );
      ...
      console.error('[PORTAL API /rolerequest/submit]', err.message);
  ...
  router.get('/leo/search', portalAuth, requireLeo, async (req, res) => {
    try {
      ...
      console.error('[PORTAL API /leo/search]', err.message);
      res.status(500).json({ error: 'Internal error' });
    }
  });
